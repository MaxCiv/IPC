...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	LPTSTR lpszPipename = TEXT("\\\\192.168.0.106\\pipe\\$$MyPipe$$"); // Имя создаваемого канала Pipe
	...
	while (1)
	{
		...
		swprintf(chBuf, BUFSIZE, L"[%5d]: ", GetCurrentThreadId());
		...
		swprintf(chBuf, BUFSIZE, L"[%5d]: %s", GetCurrentThreadId(), chBufN);
